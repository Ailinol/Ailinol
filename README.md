<p align="center">
  <img src="https://raw.githubusercontent.com/MicaelliCode/micaellicode/master/Assets/GIF/hi.gif" width="50px" alt="Hello GIF" />
</p>

# <h1 align="center">Olá, eu sou o Liliano Ricardo Licumba</h1>

<p align="center">
  <a href="https://github.com/Liliano">
    <img src="https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white" alt="GitHub Badge">
  </a>
  <a href="https://www.linkedin.com/in/seu-linkedin/">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn Badge">
  </a>
  <a href="mailto:seu-email@example.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email Badge">
  </a>
</p>

<h3 align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=28&pause=1000&color=33FF33&center=true&vCenter=true&width=600&lines=BUILDING+REAL+SOLUTIONS;LEVELING+UP+CYBERSECURITY;ENGINEERING+AI/LLM;FROM+MOZAMBIQUE+TO+THE+WORLD" alt="Typing Animation" />
</h3>

---

## 🚀 Onde o Código Ganha Vida: O Que Estou Construindo & Aprendendo

Minha jornada é sobre transformar desafios em soluções. Focado em aprimorar minhas bases e aplicar tudo em sistemas reais que geram impacto.

- 🔧 **Java & Spring Boot** — Construindo sistemas robustos (folha de pagamento, gestão de produtos, educacionais).
- 🖥️ **Java Swing** — Desenvolvendo GUIs funcionais para casos de uso reais.
- 🧠 **AI / LLM Engineering** — Criando assistentes inteligentes locais (Projeto *SEMENETE*).
- 🎓 **Sistemas Educacionais** — Assistente inteligente focado na UEM + sistemas de gestão escolar.
- 🔐 **Cybersecurity** — Do básico à prática (futuro em red-team/blue-team).

_Eu codifico para resolver os problemas ao meu redor, especialmente no contexto de Moçambique, onde a tecnologia pode ter um impacto real e imediato._

---

## 🛠️ Meu Arsenal: Tech Stack

<p align="left">
  <img src="https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=java&logoColor=white" alt="Java Badge" />
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white" alt="Spring Boot Badge" />
  <img src="https://img.shields.io/badge/Java_Swing-lightgrey?style=for-the-badge&logo=apache&logoColor=white" alt="Java Swing Badge" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python Badge" />
  <img src="https://img.shields.io/badge/Pascal-D40027?style=for-the-badge&logo=delphi&logoColor=white" alt="Pascal Badge" />
</p>

<p align="left">
  <img src="https://img.shields.io/badge/LLM_Engineering-FF4D00?style=for-the-badge&logo=openai&logoColor=white" alt="LLM Engineering Badge" />
  <img src="https://img.shields.io/badge/Cybersecurity-1F7B1F?style=for-the-badge&logo=kalilinux&logoColor=white" alt="Cybersecurity Badge" />
</p>

<p align="left">
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" alt="Git Badge" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub Badge" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" alt="Linux Badge" />
  <img src="https://img.shields.io/badge/IntelliJ_IDEA-000000?style=for-the-badge&logo=intellij-idea&logoColor=white" alt="IntelliJ IDEA Badge" />
  <img src="https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white" alt="Jupiter Lab" />
</p>

---

## 📌 Projetos em Andamento: Minhas Missões

### 🔹 **Sistema de Administração Escolar (UEM Project)**
Um sistema robusto para gerir pessoal escolar, certificados, relatórios e distribuição de materiais académicos.
- **Funcionalidades:** Dois níveis de acesso (Direção & Professores), armazenamento de arquivos binários, interface em Java Swing.

### 🔹 **Local Intelligent Assistant — “Semente”**
Um assistente de IA privado e local que aprende com o utilizador, mantendo-se offline.
- **Foco:** LLM engineering + personalização + privacidade.

### 🔹 **TARIMBA**
Um sistema de experiência digital para ajudar jovens sem oportunidades a exibir as suas habilidades e conseguir empregos.

### 🔹 **Product Management System (Java Swing)**
Categorias, geração automática de IDs, persistência de dados e uma GUI limpa e funcional.

---

## 📈 Código em Números: Minhas Estatísticas do GitHub

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=SEU_USERNAME&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" alt="Liliano's GitHub Stats" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=SEU_USERNAME&layout=compact&theme=tokyonight&hide_border=true" alt="Top Languages" />
</p>

---

## 🧩 O Lado Fun: Além do Terminal

- Eu depuro a vida como depuro o código.
- Eu realmente gosto de aprender coisas difíceis da maneira mais difícil.
- Minha vida social às vezes lança exceções, mas sempre encontro um `catch`!

---

<p align="center">
  <img src="https://user-images.githubusercontent.com/74038190/229235071-33230a38-c642-4f05-ae1c-b26a117b960a.gif" width="50" height="50"/>
</p>
